...
import { createPolymarketGammaProviderAdapter } from "../providers/polymarket/gamma.js";

const DEFAULT_ODDS_SNAPSHOT_FILE = "fixtures/snapshots/latest/raw/the-odds-api-h2h.json";
const DEFAULT_POLYMARKET_SNAPSHOT_FILE = "fixtures/snapshots/latest/raw/polymarket-worldcup.json";
...
function readWrappedSnapshotPayload(filePath) {
  ...
  if (raw && typeof raw === "object" && Array.isArray(raw.body)) {
    return raw.body;
  }
...
export async function tryReplayPredictionMarketsFromSnapshot(config, options = {}) {
  const replayFile =
    options.replayFile ||
    process.env.PREDICTION_SNAPSHOT_REPLAY_FILE ||
    DEFAULT_POLYMARKET_SNAPSHOT_FILE;

  const absolutePath = path.isAbsolute(replayFile) ? replayFile : path.resolve(projectRoot, replayFile);
  if (!existsSync(absolutePath)) {
    return null;
  }
  ...
  const adapter = createPolymarketGammaProviderAdapter(config);
  const originalFetch = adapter.fetchRawEvents.bind(adapter);
  adapter.fetchRawEvents = async () => payload;

  try {
    const predictionBoard = await adapter.fetchNormalizedPredictionMarkets();
    if (!predictionBoard.length) {
      return null;
    }
    ...
    return {
      ...
      predictionBoard,
      providerHealth: {
        polymarketStatus: "snapshot_replay",
        polymarketRows: predictionBoard.length,
        polymarketSnapshotFile: absolutePath,
        polymarketReplayReason: options.reason || "prediction market fetch failed",
      },
    };
  } finally {
    adapter.fetchRawEvents = originalFetch;
  }
}
