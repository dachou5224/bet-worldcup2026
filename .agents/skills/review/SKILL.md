...
CODEX_REVIEWS_CFG=$(~/.agents/skills/gstack/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.agents/skills/gstack/bin/gstack-config set codex_reviews enabled
```
...
```bash
~/.agents/skills/gstack/bin/gstack-config set codex_reviews disabled
```
...
```bash
~/.agents/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
