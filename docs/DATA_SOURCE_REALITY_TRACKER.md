...
└── raw/
    ...
    └── polymarket-worldcup.json   # `fetch:polymarket-snapshot` 或 snapshot 时 VPS 回退
    └── sporttery-football-match-list.json   # `fetch:jingcai-webapi` 体彩官网 raw
...
| 竞彩官方盘 | `providers/jingcai/official-feed.js` | `real` / `file` / `webapi` | 是 | 是（官方 SP 赔率） | webapi / file 快照 | 见 [SPORTTERY_JINGCAI_WEBAPI.md](./SPORTTERY_JINGCAI_WEBAPI.md) |
| Polymarket | `providers/polymarket/gamma.js` | `real`（VPS SSH 回退） | 是（sentiment only） | **否** | VPS / 独立脚本 | `sentiment_only=true`；本地 Gamma 常超时 |
...
```bash
npm run fetch:jingcai-webapi   # 体彩 webapi → latest/jingcai-official-feed.json
...
# B-4：本地 Gamma 不可达时，经美国 VPS SSH 拉取 Polymarket raw 快照
POLYMARKET_VPS_SSH_HOST=nice-ai-LZ npm run fetch:polymarket-snapshot

# 或在 snapshot 时启用 Polymarket + VPS 自动回退
POLYMARKET_PUBLIC_ENABLED=true POLYMARKET_VPS_SSH_HOST=nice-ai-LZ npm run snapshot:providers

# B-3 semi-auto：Gemini 生成竞彩草稿 + 与 latest diff（须人工复核后再写入 latest）
GEMINI_API_KEY=... GEMINI_HTTPS_PROXY=127.0.0.1:3213 npm run draft:jingcai-gemini
```
...
1. 竞彩官方盘主路径为 **体彩 webapi**（`npm run fetch:jingcai-webapi`）；`latest/jingcai-official-feed.json` 的 `source=sporttery_webapi`。Gemini 仅 semi-auto 草稿，不得直写 latest。
2. Polymarket 仅 sentiment，不进 direct EV。本地可设 `POLYMARKET_HTTPS_PROXY` 或 `HTTPS_PROXY`（如 `127.0.0.1:3213`）经 VPN 直连 Gamma；不可达时仍可用 `POLYMARKET_VPS_SSH_HOST` + `fetch:polymarket-snapshot`。
...
7. 如果要先打通单场黄金路径，直接使用 `MARKET_DATA_MODE=replay` + `LIVE_DATA_MODE=replay`，然后跑 `npm run qa:pipeline-replay` 或 `npm run pipeline:replay`。
